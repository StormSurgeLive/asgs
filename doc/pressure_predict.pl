...
for (my $i=1; $i<$num_real; $i++ ) {
   ...
   for ( my $j=$i; $j<$num_real; $j++ ) { 
      ...
      if ( $method eq "twoslope" ) {
      ...
      } elsif ( $method eq "knaffzehrgeneral" ) {
         if ( $trackfile_type ne "nws19" ) {
            stderrMessage("ERROR","The $method method was specified but the trackfile type was set to $trackfile_type, whereas the 'nws19' trackfile type is required for this method.");
         }
         ...
         if ( $avgr34[$j] != -99999 ) {
            ...
            my $v500 = 0.1 * $avgr34[$j]; 
            my $S = $v500/$v500c;
            my $penv = 1013.0;
            $pcf[$j] = 23.286 - 0.483*$vsrm - ($vsrm/24.254)**2 - 12.587*$S - 0.483*$lats[$j] + $penv;
         } else {
            # if we couldn't get a size estimate, just use dvorak
            $pcf[$j] = 1015 - ($vmaxes[$j]/3.92*0.51444444)**(1.0/0.644);
         }
      ...
      } elsif ( $method eq "dynkz" ) {
         # Dynamic Knaff-Zehr, using decaying environmental pressure
         # must convert from knots to m/s to use the formula
         if ( $pns[$i] < 1010 ) {
            $pcf[$j] = $pns[$i] - ($vmaxes[$j]/2.3*0.51444444)**(1.0/0.76);
         } else {
