...
while ( !$dl ) {
    ...
    if ( $ftpsite eq "filesystem" ) {
        ...
        if ( -e $hdir && -d $hdir ) {
            ...
            stderrMessage( "ERROR", "Get '$hindcastfile' failed: " . " the local directory $hdir does not exist, or is not a directory." );
            next;
        }
    }
    else {
    ...
    if ( $trigger eq "ftp" ) {
        ...
        unless ($fcDirSuccess) {
            stderrMessage(
                "ERROR",
                "ftp: Cannot change working directory to '$fdir': " . $ftp->message
            );
            next;
        }
        $fcDl = $ftp->get($forecastfile);
        unless ($fcDl) {
            stderrMessage( "ERROR", "ftp: Get '$forecastfile' failed: " . $ftp->message );
            next;
        }
