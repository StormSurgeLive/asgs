...
#trap read debug
#----------------------------------------------------------------
# downloadCOAMPSTC.sh: Subroutine that polls the MetGet service
# to determine status of COAMPS-TC data and then subsets
# and downloads it via web client.
#----------------------------------------------------------------
...
downloadCOAMPSTC()
{
    THIS="asgs_main.sh>downloadCOAMPSTC.sh"
    CURRENT_STATE="WAIT"
    RMQMessage "INFO" "$CURRENT_EVENT" "$THIS>$ENSTORM" "$CURRENT_STATE"  "Downloading COAMPS-TC meteorological data for $ENSTORM."
    logMessage "$ENSTORM: $THIS: Downloading COAMPS-TC meteorological data."
    cd $RUNDIR 2>> ${SYSLOG}
    #
    # if there isn't an archive directory for COAMPS-TC data inside
    # the instance directory, make one
    instanceCoampsTcDir=$RUNDIR/coamps-tc
    if [[ ! -d $instanceCoampsTcDir ]]; then
       mkdir -p $instanceCoampsTcDir 2>> $SYSLOG
    fi
    # if there isn't an archive directory for GFS data in the
    # ASGS WORK directory, make one
    platformCoampsTcDir=$WORK/coamps-tc
    if [[ ! -d $platformCoampsTcDir ]]; then
       mkdir -p $platformCoampsTcDir 2>> $SYSLOG
    fi
    #
    # N O W C A S T
    if [[ $stage == "NOWCAST" ]]; then
        ...
        # determine the status of the latest COAMPS-TC cycle posted by NRL Monterey,
        ...
        while [[ $latestCycle -le $lastCycle ]]; do
            ...
            ((TRIES++))
            # getting coamps-tc status
            get_metget_status.pl --storm $STORM --backgroundmet $BACKGROUNDMET --tropicalcyclone $TROPICALCYCLONE > coampstc_status.json 2>> $SYSLOG
            if [[ $? != 0 ]]; then
                warn "$THIS: Failed to get status of COAMPS-TC cycles with get_metget_status.pl."
                RMQMessage "INFO" "$CURRENT_EVENT" "$THIS>$ENSTORM" "$CURRENT_STATE" \
                    "Waiting on NCEP data for $ENSTORM. Sleeping 60 secs (TRY=$TRIES) ..."
                sleep 60
                continue
            fi
            bashJSON.pl --key cyclelist --last < coampstc_status.json > latestCycle 2>> $SYSLOG
            if [[ $? != 0 ]]; then
                warn "$THIS: Failed to extract the latest COAMPS-TC cycle from coampstc_status.pl.json with bashJSON.pl"
                RMQMessage "INFO" "$CURRENT_EVENT" "$THIS>$ENSTORM" "$CURRENT_STATE" \
                    "Waiting on NCEP data for $ENSTORM. Sleeping 60 secs (TRY=$TRIES) ..."
                sleep 60
                continue
        ...
        select_nam_nowcast.pl < coampstc_status.json > select_coampstc_nowcast.json 2>> $SYSLOG
        if [[ $? != 0 ]]; then
            warn "$THIS: Failed to select the proper COAMPS-TC cycle to end the nowcast with select_nam_nowcast.pl."
        fi
