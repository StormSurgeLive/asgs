...
if ( abs($basenws) == 19 || abs($basenws) == 20 || abs($basenws) == 8 || abs($basenws) == 30 ) {
   ...
   vortexModelParameters($basenws);
   # for getting the OWI wind time increment for blended winds
   # and appending it to the wtiminc line
   if ( abs($basenws) == 30 ) {
      $wtiminc_line .= " $p->{meteorology}->{wtiminc} $p->{meteorology}->{blending}->{pure_vortex_radius} $p->{meteorology}->{blending}->{pure_background_radius}";
   }
} elsif ( abs($basenws) == 12 ) {
   owiParameters();
   $wtiminc_line = "$p->{meteorology}->{wtiminc}";
} elsif ( defined $specifiedRunLength ) {
   ASGSUtil::stderrMessage("DEBUG","The duration of this $p->{scenario} run is specially defined.");
   customParameters();
...
if ( $nws ne "0" && $p->{wave_coupling}->{waves} eq "on" && $p->{output}->{inventory} ne "metonly"  ) {
...
if ( $nws ne "0" ) {
   ...
   if ( abs($basenws) == 19 || abs($basenws) == 20 || abs($basenws) == 8 || abs($basenws) == 30 ) {
      ...
      if ( abs($basenws) == 30 ) {
         printf RUNPROPS "adcirc.control.numerics.meteorology.purevortex : $p->{meteorology}->{blending}->{pure_vortex_radius}\n";
         printf RUNPROPS "adcirc.control.numerics.meteorology.purebackground : $p->{meteorology}->{blending}->{pure_background_radius}\n";
      }
   }
}
