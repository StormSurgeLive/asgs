...
delete() {
  case "${1}" in
    ...
    ;;
    *)
      echo "'delete' requires 2 parameters for 'adcirc' and 'profile' specifying which ADCIRC build or profile to delete. All others do not: 'config', 'statefile'."
      return
  esac
}
...
sq() {
  ...
  if [[ $(echo 'supermic queenbee queenbeeC' | grep -c "$platform") -gt 0 ]]; then
    SQ=$(showq | grep $USER)
  elif [[ $(echo 'frontera stampede2 lonestar5 hatteras' | grep -c "$platform") -gt 0 ]]; then
    SQ=$(squeue -u $USER)
  else
