...
sub _get_env_summary {
  ...
  GETENV:
    foreach my $envar ( keys %$AFFECTED_ENV_VARS ) {
        ...
        next GETENV if grep { m/^$envar$/ } (qw/HDF5_USE_FILE_LOCKING/);
...
sub get_steps {
    ...
    my $steps = [
	    ...
	    command             => qq{bash init-cpra-postproc.sh $asgs_install_path $asgs_compiler},
	    clean               => qq{bash init-cpra-postproc.sh $asgs_install_path clean},
	    ...
	    command             => qq{bash init-output.sh $asgs_install_path $asgs_compiler},
	    clean               => qq{bash init-output.sh $asgs_install_path clean},
	    ...
	    command             => qq{bash init-util.sh $asgs_install_path $asgs_compiler},
	    clean               => qq{bash init-util.sh $asgs_install_path clean},
	    ...
	    command             => qq{bash init-input-nodalattr.sh $asgs_install_path $asgs_compiler},
	    clean               => qq{bash init-input-nodalattr.sh $asgs_install_path clean},
