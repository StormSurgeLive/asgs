...
if ($offsetStartSec > $offsetFinishSec) {
    ...
    if ( defined $hstime && $offsetFactorAtPreviousRunFinish != 0.0 ) {
        ...
        $runProps{"forcing.offset.modified.reason"} = "Offset was configured to be off in this scenario but it was turned on in the previous run. This will cause numerical issues in ADCIRC, so it will be turned on in the current scenario at a constant value equal to the value from the previous run.");       
        ...
        $runProps{"forcing.offset.modified.offsetfactor.atrunstart"} = $offsetFactorAtRunStart;
        $offsetFactorAtRunFinish = $offsetFactorAtPreviousRunFinish;           
        $runProps{"forcing.offset.modified.offsetfactor.atrunfinish"} = $offsetFactorAtRunFinish;
        $offsetFactorStart = $offsetFactorAtPreviousRunFinish;
        $runProps{"forcing.offset.modified.offsetfactorstart"} = $offsetFactorStart;
        $offsetFactorFinish = $offsetFactorAtPreviousRunFinish;           
        $runProps{"forcing.offset.modified.offsetfactorfinish"} = $offsetFactorFinish;
...
$runProps{"forcing.offset.offsetfactorfinish.seconds"} = $offsetFinishSec;
   ...
   if ( $runProp{"forcing.offset"} eq "dynamic"  ) {
      ...
      #
         ...
         # 
         # if the Operator wants ASGS to compute the right starting
         # factor for the offset, set it to zero since no offset was
         # applied in the previous run. 
         if ( $offsetFactorStart eq "auto" ) {
            $offsetFactorStart = 0.0;
         } 
