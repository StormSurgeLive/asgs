...
unless ( $ncpu ) { die "ERROR: penguin.pbs.pl: The number of CPUs was not specified."; }
unless ( $ppn ) { die "ERROR: penguin.pbs.pl: The number of processors per node was not specified."; }

# the B30 queue use 28 ppn
$ppn=28 if $queuename eq 'B30';
# the S30 queue use 40 ppn
$ppn=40 if $queuename eq 'S30';
#
