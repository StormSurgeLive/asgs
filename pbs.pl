...
unless ( $ncpu ) { die "ERROR: pbs.pl: The number of CPUs was not specified."; }
unless ( $ppn ) { die "ERROR: pbs.pl: The number of processors per node was not specified."; }
#
...
#
open(TEMPLATE,"<$qscript") || die "ERROR: pbs.pl: Can't open $qscript file for reading as a template for the queue submission script.";
#
while(<TEMPLATE>) {
