MODULE DATA
        ...
        CHARACTER(LEN=85)   :: PlotLabel2
        CHARACTER(LEN=50)   :: PlotLabelFile
        CHARACTER(LEN=50)   :: PlotLabelInputFile
        ...
        INTEGER             :: IfPlotPartTrack
        INTEGER             :: IfPlotInitialPositions
...
PROGRAM FigureGen
        ...
        IF(MyRank.EQ.0)THEN
            ...
            WRITE(*,'(A)',ADVANCE="YES") " "
            WRITE(*,'(A)',ADVANCE="YES") "---------------------------------------------"
            WRITE(*,'(A)',ADVANCE="YES") "FigureGen                          2009/11/18"
            WRITE(*,'(A)',ADVANCE="YES") " "
        ...
        IF(MyRank.EQ.0)THEN
           ...
           DO I=1,NumProcs-1
               ...
               !...Occupy Processor for a fraction of a second
               DO J=1,10000
                   JunkR=JunkR+(0.5**J)**2
               ENDDO
               JunkR=0
       ...
       ENDIF
        ...
        IF(MyRank.EQ.0)THEN
            ...
            IF((IfPlotFilledContours.GT.0).OR.(IfPlotContourLines.GT.0))THEN
                IF(TRIM(ContourFileFormat).EQ."ADCIRC-OUTPUT")THEN
        ...
        IF(NumRecords.EQ.0)THEN
         ...
         ENDIF
              ...
              WRITE(*,*) "made it here  1, NumRecords = ",NumRecords
                ...
                IF(WorkingRecord.GT.0)THEN
                    ...
                    IF(Verbose.GE.3)THEN
                        IF((IfGoogle.EQ.0).AND.(IfGIS.EQ.0))THEN
                            WRITE(*,'(A,I4.4,A,I4.4,A)') "Processor ",MyRank," started record ",Record,"."
                        ELSE
                            WRITE(*,'(A,I4.4,A,I4.4,A,I2.2,A,I2.2,A,I2.2,A)') "Processor ",MyRank," started record ",Record, &
                                    ", layer ",IL1,", cell ",IL2,"/",IL3,"."
                        ENDIF
                    ENDIF

                    IF((IfPlotFilledContours.GT.0).OR.(IfPlotContourLines.GT.0).OR.(IfPlotPartTrack.GT.0))THEN
                        ...
                        IF((IL1.EQ.1).AND.(IL2.EQ.1).AND.(IL3.EQ.1))THEN
                            ...
                            ENDIF
                            IF(IfPlotPartTrack.GT.0)THEN
                                WRITE(100,'(A)') TRIM(PartTrackInitial)
                                WRITE(100,'(A)') TRIM(PartTrackSnap)
                            ENDIF
                            WRITE(100,'(A)') " "
                            CLOSE(100)
                        ENDIF
                        ...
                        filestatus: DO
                            ...
                            IF(FileExists)THEN
                                ...
                                ENDIF
                                IF(IfPlotPartTrack.GT.0)THEN
                                    READ(UNIT=100,FMT='(A)',ERR=978,END=978) PartTrackInitial
                                    READ(UNIT=100,FMT='(A)',ERR=978,END=978) PartTrackSnap
                                ENDIF
                                CLOSE(100)
                                EXIT filestatus
 978                            CONTINUE
                    ...
                    IF(Verbose.GE.2)THEN
                        IF(NumLayers.EQ.1)THEN
                            WRITE(*,'(A,I4.4,A,I4.4,A)') "Processor ",MyRank," completed record ",Record,"."
                        ELSE
                            WRITE(*,'(A,I4.4,A,I4.4,A,I2.2,A,I2.2,A,I2.2,A)') "Processor ",MyRank," completed record ",Record, &
                                    ", layer ",IL1,", cell ",IL2,"/",IL3,"."
                        ENDIF
                    ENDIF
        ...
        IF((MyRank.EQ.0).AND.(IfGIS.EQ.2))THEN
            ...
            INQUIRE(FILE=TRIM(AlphaLabel)//".zip",EXIST=FileExists)
            IF(FileExists)THEN
                CALL SYSTEM("rm "//TRIM(AlphaLabel)//".zip")
            ENDIF
            ...
            DO I=1,NumRecords
                ...
                CALL SYSTEM("zip -qr "//TRIM(AlphaLabel)//" "//TRIM(RecordLabel))
                CALL SYSTEM("rm -fR "//TRIM(RecordLabel))
                CALL SYSTEM("rm -fR "//TRIM(RecordLabel)//"*.ps")
            ENDDO

            IF(INDEX(ContourFileFormat,"GRID-DECOMP").LE.0)THEN
                CALL SYSTEM("zip -qr "//TRIM(AlphaLabel)//" Scale.jpg")
...
SUBROUTINE CreateCPTFiles
 ...
 9612       CONTINUE
        ...
        DO K=1,3
            ...
            IF((K.EQ.1).AND.(TRIM(ContourFileFormat).NE."HWM-CSV").AND.(TRIM(ContourFileFormat).NE."PART-TRACK"))THEN
            ...
            IF(TRIM(ContourFileFormat).EQ."ADCIRC-OUTPUT")THEN
                ...
                loopminmax1: DO J=1,NumRecsLocal 
                    ...
                    IF(J.LT.RecordsList(CounterLocal))THEN
                        ...
                        CALL ReadTimeStamp(19,TRIM(ContourFile1),JunkR,JunkR2,NumNodes1,DefaultValue)
                ...
                ELSEIF(TRIM(ContourFileFormat).EQ."GRID-SIZE")THEN
                    ...
                    DO I=1,NumElemsGlobal
                        ...
                        DO J=1,NumNodesLocal
                            ...
                            ELSEIF(NC(I,2).EQ.XYZNodes(J))THEN
                            ...
                            ELSEIF(NC(I,3).EQ.XYZNodes(J))THEN
            ...
            IF(TRIM(ContourFileFormat).EQ."ADCIRC-OUTPUT")THEN
                ...
                loopminmax2: DO J=1,NumRecsLocal 
                    ...
                    IF(J.LT.RecordsList(CounterLocal))THEN
                        ...
                        CALL ReadTimeStamp(19,TRIM(ContourFile1),JunkR,JunkR2,NumNodes1,DefaultValue)
                        CALL ReadTimeStamp(23,TRIM(ContourFile2),JunkR,JunkR2,NumNodes2,DefaultValue)
...
SUBROUTINE FindVectorScaleMag
        ...
        loopvectorscale: DO J=1,NumRecsLocal 
            ...
            IF(J.LT.RecordsList(CounterLocal))THEN
                ...
                CALL ReadTimeStamp(20,TRIM(VectorFile),JunkR,JunkR2,NumNodes1,DefaultValue)
...
!... to the edges of the map.
      ...
      IF(ABS(LongWLocal)<100.0)THEN
         WRITE(UNIT=XMin,FMT='(F12.8)') LongWLocal
      ELSE
         WRITE(UNIT=XMin,FMT='(F13.8)') LongWLocal
      ENDIF
      IF(ABS(LongELocal)<100.0)THEN
         WRITE(UNIT=XMax,FMT='(F12.8)') LongELocal
      ELSE
         WRITE(UNIT=XMax,FMT='(F13.8)') LongELocal
      ENDIF
      IF(ABS(LatSLocal)<10.0)THEN
         WRITE(UNIT=YMin,FMT='(F10.8)') LatSLocal
      ELSE
         WRITE(UNIT=YMin,FMT='(F11.8)') LatSLocal
      ENDIF
      IF(ABS(LatNLocal)<10.0)THEN
         WRITE(UNIT=YMax,FMT='(F10.8)') LatNLocal
      ELSE
         WRITE(UNIT=YMax,FMT='(F11.8)') LatNLocal
      ENDIF
      ...
      IF(INDEX(ContourFileFormat,"GRID-DECOMP").LE.0.AND.                        &
         ((IfPlotPartTrack.EQ.0).AND.(IfPlotFilledContours.GT.0)) )THEN
      ...
      WRITE(UNIT=35,FMT='(A)') "            <north>"//TRIM(YMax)//"</north>"
      WRITE(UNIT=35,FMT='(A)') "            <south>"//TRIM(YMin)//"</south>"
      WRITE(UNIT=35,FMT='(A)') "            <east>"//TRIM(XMax)//"</east>"
      WRITE(UNIT=35,FMT='(A)') "            <west>"//TRIM(XMin)//"</west>"
      WRITE(UNIT=35,FMT='(A)') "         </LatLonAltBox>"
      WRITE(UNIT=35,FMT='(A)') "      </Region>"
 ...
 9722          FORMAT(A,I4.4,A,I2.2,A,I2.2,A,I2.2)
      ...
      IF(Verbose.GE.3)THEN
         WRITE(*,9723) "Processor ",MyRank," wrote the local Google KML file for record ",WorkingRecord, &
                       ", layer ",IL1,", cell ",IL2,"/",IL3,"."
      ENDIF
...
!Casey 100606: Changes start here.
      ...
      IF(Verbose.GE.3)THEN
         WRITE(*,9724) "Processor ",MyRank," wrote the Google KMZ file."
      ENDIF
        ...
        DO I=1,NumElemsLocal
            ...
            IF(IfPlotGrid.EQ.1)THEN
                ...
                IF(TRIM(ColorLines).EQ."GRID-BATH")THEN
                    ZVal = 0.5D0*(BathGlobal(NC(NEList(I),1))+BathGlobal(NC(NEList(I),2)))
                ...
                IF(TRIM(ColorLines).EQ."GRID-BATH")THEN
                    ZVal = 0.5D0*(BathGlobal(NC(NEList(I),2))+BathGlobal(NC(NEList(I),3)))
                    ZVal = ZVal * ContourConversionFactor
                    WRITE(UNIT=31,FMT='(A,F15.6)') "> -Z",ZVal
                ELSEIF(INDEX(ColorLines,"GRID-DECOMP").GT.0)THEN
                ...
                IF(TRIM(ColorLines).EQ."GRID-BATH")THEN
                    ZVal = 0.5D0*(BathGlobal(NC(NEList(I),3))+BathGlobal(NC(NEList(I),1)))
                    ZVal = ZVal * ContourConversionFactor
                    WRITE(UNIT=31,FMT='(A,F15.6)') "> -Z",ZVal
                ELSEIF(INDEX(ColorLines,"GRID-DECOMP").GT.0)THEN
...
SUBROUTINE ReadInputFile
        ...
        CHARACTER(LEN=85) :: TempC1
        CHARACTER(LEN=50) :: TempC2
        CHARACTER(LEN=85) :: TempC3
        ...
        WRITE(*,*) IfAddPlotLabel, PlotLabel
        ELSEIF(TempC(1:1).EQ."2")THEN
            TempC2 = TempC(1:INDEX(TempC,",")-1)
            READ(UNIT=TempC2,FMT=*) IfAddPlotLabel
            PlotLabelInputFile = TempC(INDEX(TempC,",")+1:LEN_TRIM(TempC))
        WRITE(*,*) IfAddPlotLabel, PlotLabelInputFile
        ENDIF
        ...
        ELSE
            ...
            IF(.NOT.FileExists)THEN
                IF(MyRank.EQ.0)THEN
                    WRITE(*,'(A)') "FATAL ERROR: The first contour file does not exist. FigureGen is quitting."
                ENDIF
#ifdef CMPI
                CALL MPI_FINALIZE(IERR)
#endif
            ...
            IF(.NOT.FileExists)THEN
                IF(MyRank.EQ.0)THEN
                    WRITE(*,'(A)') "FATAL ERROR: The second contour file does not exist. FigureGen is quitting."
                ENDIF
#ifdef CMPI
                CALL MPI_FINALIZE(IERR)
#endif
        ...
        IF(TRIM(ContourFileFormat).EQ."0")THEN
            IF(MyRank.EQ.0)THEN
                WRITE(UNIT=*,FMT='(A)') "WARNING: ContourFileFormat = 0 and reset to ADCIRC-OUTPUT."
            ENDIF
            ContourFileFormat = "ADCIRC-OUTPUT"
        ENDIF
        IF(TRIM(ContourFileFormat).EQ."1")THEN
            IF(MyRank.EQ.0)THEN
                WRITE(UNIT=*,FMT='(A)') "WARNING: ContourFileFormat = 1 and reset to ADCIRC-OUTPUT."
            ENDIF
            ContourFileFormat = "ADCIRC-OUTPUT"
        ENDIF
        ...
        IF(TRIM(ContourFileFormat).EQ."OUTPUT-FULL")THEN
            IF(MyRank.EQ.0)THEN
                WRITE(UNIT=*,FMT='(A)') "WARNING: ContourFileFormat = OUTPUT-FULL and reset to ADCIRC-OUTPUT."
            ENDIF
            ContourFileFormat = "ADCIRC-OUTPUT"
        ENDIF
        IF(TRIM(ContourFileFormat).EQ."OUTPUT-SPARSE")THEN
            IF(MyRank.EQ.0)THEN
                WRITE(UNIT=*,FMT='(A)') "WARNING: ContourFileFormat = OUTPUT-SPARSE and reset to ADCIRC-OUTPUT."
            ENDIF
            ContourFileFormat = "ADCIRC-OUTPUT"
        ENDIF
        ...
        IF((TRIM(ContourFileFormat).EQ."GRID-SIZE").AND.(IfPlotContourLines.EQ.2))THEN
            ...
            STOP
        ENDIF
        IF(TRIM(ContourFileFormat).EQ."HWM-CSV")THEN
            IF(IfPlotFilledContours.GT.0)THEN
                IF(MyRank.EQ.0)THEN
                    WRITE(UNIT=*,FMT='(A)') "WARNING: ContourFileFormat = HWM-CSV, so IfPlotFilledContours reset to 0."
                ENDIF
                IfPlotFilledContours = 0
            ENDIF
            IF(IfPlotContourLines.GT.0)THEN
                IF(MyRank.EQ.0)THEN
                    WRITE(UNIT=*,FMT='(A)') "WARNING: ContourFileFormat = HWM-CSV, so IfPlotContourLines reset to 0."
                ENDIF
        ...
        IF(IfPlotVectors.EQ.1)THEN
            ...
            IF(.NOT.FileExists)THEN
                IF(MyRank.EQ.0)THEN
                    WRITE(*,'(A)') "FATAL ERROR: The vector file does not exist. FigureGen is quitting."
                ENDIF
#ifdef CMPI
                CALL MPI_FINALIZE(IERR)
#endif
        ...
        IF(TRIM(VectorFileFormat).EQ."0")THEN
            IF(MyRank.EQ.0)THEN
                WRITE(UNIT=*,FMT='(A)') "WARNING: VectorFileFormat = 0 and reset to ADCIRC-OUTPUT."
            ENDIF
            VectorFileFormat = "ADCIRC-OUTPUT"
        ENDIF
        IF(TRIM(VectorFileFormat).EQ."1")THEN
            IF(MyRank.EQ.0)THEN
                WRITE(UNIT=*,FMT='(A)') "WARNING: VectorFileFormat = 1 and reset to ADCIRC-OUTPUT."
            ENDIF
            VectorFileFormat = "ADCIRC-OUTPUT"
        ENDIF
        IF(TRIM(VectorFileFormat).EQ."OUTPUT-FULL")THEN
            IF(MyRank.EQ.0)THEN
                WRITE(UNIT=*,FMT='(A)') "WARNING: VectorFileFormat = OUTPUT-FULL and reset to ADCIRC-OUTPUT."
            ENDIF
            VectorFileFormat = "ADCIRC-OUTPUT"
        ENDIF
        IF(TRIM(VectorFileFormat).EQ."OUTPUT-SPARSE")THEN
            IF(MyRank.EQ.0)THEN
                WRITE(UNIT=*,FMT='(A)') "WARNING: VectorFileFormat = OUTPUT-SPARSE and reset to ADCIRC-OUTPUT."
            ENDIF
            VectorFileFormat = "ADCIRC-OUTPUT"
        ENDIF
        ...
        ENDIF
        READ(UNIT=11,FMT=*) MinVectorToPlot

        READ(UNIT=11,FMT='(A1)')  JunkC ! PARAMETERS FOR PARTICLE TRACKING

        READ(UNIT=11,FMT=*)       IfPlotPartTrack
        ...
        READ(UNIT=11,FMT='(A40)') IfPlotInitialPositions
        ...
        IF((ABS(LongW-LongE).LT.ABS(LatN-LatS)).AND.(Width.GT.4.))THEN
            ...
            ENDIF
        ENDIF
        READ(UNIT=11,FMT=*)       Buffer,Buffer_scale
        ...
        ELSE
            ...
            typesearch_outer: DO
                ...
                ELSE
                    ...
                    ELSE
                        ...
                        WRITE(*,'(A)') 'WARNING: No raster file will be created.'
                    ENDIF
            ...
            IF((DoJPG.EQ.0).AND.(DoPNG.EQ.0).AND.(DoPDF.EQ.0).AND. &
               (DoEPS.EQ.0).AND.(DoBMP.EQ.0).AND.(DoTIFF.EQ.0))THEN
                     WRITE(*,'(A)') 'WARNING: No default raster. '
             !RJWCOMMENTOUT   !   DoJPG = 1
            ENDIF
        ENDIF
        ...
        ENDIF
        IF(IfGoogle.EQ.1)THEN
            IF((IfAddPlotLabel.EQ.1).OR.(IfAddPlotLabel.EQ.2))THEN
                IfAddPlotLabel = 0
                IF(MyRank.EQ.0)THEN
        ...
        ELSE
            ...
            ENDIF
        ENDIF
        IF((IfPlotLogo.EQ.1).AND.(INDEX(LogoFile,".eps").LE.0).AND.((IfGoogle.EQ.0).AND.(IfGIS.EQ.0)))THEN
            IF(MyRank.EQ.0)THEN
...
!Casey 091221: This first IF statement caused problems for Mark when he tried to plot contours
!              of grid bathymetry and vectors of currents.  Maybe it can be removed?
...
!               RecordsList(I) = 1
!           ENDDO
        IF(INDEX(ContourFileFormat,"13-").GT.0)THEN
        ...
        IF(((IfPlotFilledContours.GE.1).OR.(TRIM(ColorLines).NE."DEFAULT")).AND. &
           (INDEX(ContourFileFormat,"GRID-DECOMP").LE.0).AND.(IfGIS.EQ.0))THEN
                DoScale = 1
        ENDIF
        ...
        ELSEIF((IfGoogle.GT.0).OR.(IfGIS.GT.0))THEN
            KeepOpen(1:8) = 1
        ELSEIF(TRIM(ContourFileFormat).EQ."HWM-CSV")THEN
            KeepOpen(1:7) = 1
...
SUBROUTINE ReadTimeStamp(UnitNumber, FileName, TimeReal, TimeReal2, NonDefaultNodes, DefaultValue)
 ...
 8001       CONTINUE
            IF(Verbose.GE.4)THEN
                WRITE(*,'(A,I4.4,A,A,A)') "Processor ",MyRank," is trying to read from the end of ",TRIM(FileName),"."
            ENDIF
            BACKSPACE(UnitNumber)
            CALL SLEEP(60)
...
SUBROUTINE WritePSImage(Record,IL1,IL2,IL3)
        ...
        CHARACTER(LEN=8)  :: SideBarXC_scale
        ...
        CHARACTER(LEN=1)  :: PartSizeInit
        CHARACTER(LEN=1)  :: PartSizeSnp
        ...
        REAL              :: HWMLon
        REAL              :: PartTime
        REAL              :: PartLat,PartLatI,PartLatIm1
        REAL              :: PartLon,PartLonI,PartLonIm1
        REAL              :: JunkR
        REAL              :: LatNLocal
        REAL              :: LatSLocal
        REAL              :: LongELocal
        ...
        IF(ABS(LongWLocal)<100.0)THEN
            WRITE(UNIT=XMin,FMT='(F6.2)') LongWLocal
        ELSE
            WRITE(UNIT=XMin,FMT='(F7.2)') LongWLocal
        ENDIF
        IF(ABS(LongELocal)<100.0)THEN
            WRITE(UNIT=XMax,FMT='(F6.2)') LongELocal
        ELSE
            WRITE(UNIT=XMax,FMT='(F7.2)') LongELocal
        ENDIF
        IF(ABS(LatSLocal)<10.0)THEN
            WRITE(UNIT=YMin,FMT='(F4.2)') LatSLocal
        ELSE
            WRITE(UNIT=YMin,FMT='(F5.2)') LatSLocal
        ENDIF
        IF(ABS(LatNLocal)<10.0)THEN
            WRITE(UNIT=YMax,FMT='(F5.2)') LatNLocal
        ELSE
            WRITE(UNIT=YMax,FMT='(F5.2)') LatNLocal
        ENDIF
        ...
        ENDIF
        SideBarX_scale = Width + Buffer_scale
        IF(SideBarX_scale<10.0)THEN
            WRITE(UNIT=SideBarXC_scale,FMT='(F4.2)') SideBarX_scale
        ELSE
            WRITE(UNIT=SideBarXC_scale,FMT='(F5.2)') SideBarX_scale
        ENDIF
        ...
        IF(IfPlotFilledContours.GE.1)THEN
            ...
            IF(Verbose.GE.3)THEN
                IF((IfGoogle.EQ.0).AND.(IfGIS.EQ.0))THEN
                    WRITE(*,9720) "Processor ",MyRank," wrote the filled contours for record ",Record,"."
                ELSE
                    WRITE(*,9721) "Processor ",MyRank," wrote the filled contours for record ",Record, &
                                  ", layer ",IL1,", cell ",IL2,"/",IL3,"."
                ENDIF
            ENDIF
        ...
        IF(IfPlotGrid.EQ.1)THEN
            ...
            DO IN=1,NumEdgeFiles
                ...
                ENDIF
                Line = TRIM(Line)//" "//"-R"//TRIM(XMin)//"/"//TRIM(XMax)//"/"//TRIM(YMin)//"/"//TRIM(YMax)
                IF((IfPlotFilledContours.EQ.0).AND.(IfGoogle.EQ.0).AND.(IfGIS.EQ.0))THEN
                    Line = TRIM(Line)//" "//"-Bp"//TRIM(BorderIncrementMajorC)//"f"//TRIM(BorderIncrementMinorC)// &
                                       "/s"//TRIM(BorderIncrementMajorC)//"f"//TRIM(BorderIncrementMinorC)//"WeSn"
                ENDIF
                Line = TRIM(Line)//" "//"-C"//TRIM(TempPath)//"ContourPalette.cpt"
            ...
            IF(Verbose.GE.3)THEN
                IF((IfGoogle.EQ.0).AND.(IfGIS.EQ.0))THEN
                    WRITE(*,9720) "Processor ",MyRank," wrote the underlying mesh for record ",Record,"."
                ELSE
                    WRITE(*,9721) "Processor ",MyRank," wrote the underlying mesh for record ",Record, &
                                  ", layer ",IL1,", cell ",IL2,"/",IL3,"."
                ENDIF
            ENDIF
        ...
        IF(IfPlotContourLines.GE.1)THEN
            ...
            IF(OptimizeContours.EQ.0)THEN
                ...
                ENDIF
                Line = TRIM(Line)//" "//"-R"//TRIM(XMin)//"/"//TRIM(XMax)//"/"//TRIM(YMin)//"/"//TRIM(YMax)
                IF((IfPlotFilledContours.EQ.0).AND.(IfPlotGrid.EQ.0).AND.(IfGoogle.EQ.0).AND.(IfGIS.EQ.0))THEN
                    Line = TRIM(Line)//" "//"-Bp"//TRIM(BorderIncrementMajorC)//"f"//TRIM(BorderIncrementMinorC)// &
                                       "/s"//TRIM(BorderIncrementMajorC)//"f"//TRIM(BorderIncrementMinorC)//"WeSn"
                ENDIF
                ...
                ELSE
                    ...
                    Line = TRIM(Line)//" "//"-Gd"//TRIM(ContourLabelEveryC)//"i+r"//TRIM(ContourLabelMinDistC)//"i"
                ENDIF
            ...
            IF(Verbose.GE.3)THEN
                IF((IfGoogle.EQ.0).AND.(IfGIS.EQ.0))THEN
                    WRITE(*,9720) "Processor ",MyRank," wrote the contour lines for record ",Record,"."
                ELSE
                    WRITE(*,9721) "Processor ",MyRank," wrote the contour lines for record ",Record, &
                                  ", layer ",IL1,", cell ",IL2,"/",IL3,"."
                ENDIF
            ENDIF
        ...
        IF(((IfPlotFilledContours.GE.1).OR.(TRIM(ColorLines).NE."DEFAULT").OR. & 
           (TRIM(ContourFileFormat).EQ."HWM-CSV")).AND. &
           (INDEX(ContourFileFormat,"GRID-DECOMP").LE.0).AND.(IL1.EQ.1).AND.(IfPlotScale.GT.0))THEN
               !    SideBarXC_scale=0.5
            Line = ""
            Line = TRIM(Line)//TRIM(Path)//"psscale"
            Line = TRIM(Line)//" "//"-D"//TRIM(SideBarXC_scale)//"i/"//TRIM(ContourScaleYC)//"i/" &
            ...
            IF(Verbose.GE.3)THEN
                IF((IfGoogle.EQ.0).OR.(IfGIS.GT.0))THEN
                    WRITE(*,9720) "Processor ",MyRank," wrote the scale for record ",Record,"."
                ELSE
                    WRITE(*,9721) "Processor ",MyRank," wrote the scale for record ",Record, &
                                  ", layer ",IL1,", cell ",IL2,"/",IL3,"."
                ENDIF
            ENDIF
        ...
        IF(IfPlotBoundaries.EQ.1)THEN
            ...
            IF(Verbose.GE.3)THEN
                IF((IfGoogle.EQ.0).AND.(IfGIS.EQ.0))THEN
                    WRITE(*,9720) "Processor ",MyRank," wrote the levee/road boundaries for record ",Record,"."
                ELSE
                    WRITE(*,9721) "Processor ",MyRank," wrote the levee/road boundaries for record ",Record, &
                                  ", layer ",IL1,", cell ",IL2,"/",IL3,"."
                ENDIF
            ENDIF
        ...
        IF(IfPlotCoastline.GT.0)THEN
            ...
            ENDIF
            Line = TRIM(Line)//" "//"-R"//TRIM(XMin)//"/"//TRIM(XMax)//"/"//TRIM(YMin)//"/"//TRIM(YMax)
            Line = TRIM(Line)//" "//"-W0.50p/"//TRIM(CoastlineColor)
            ...
            IF(Verbose.GE.3)THEN
                IF((IfGoogle.EQ.0).AND.(IfGIS.EQ.0))THEN
                    WRITE(*,9720) "Processor ",MyRank," wrote the coastline for record ",Record,"."
                ELSE
                    WRITE(*,9721) "Processor ",MyRank," wrote the coastline for record ",Record, &
                                  ", layer ",IL1,", cell ",IL2,"/",IL3,"."
                ENDIF
            ENDIF
        ...
        IF(IfPlotDotsLines.GT.0)THEN
            ...
            IF(Verbose.GE.3)THEN
                IF((IfGoogle.EQ.0).AND.(IfGIS.EQ.0))THEN
                    WRITE(*,9720) "Processor ",MyRank," wrote the dots/lines for record ",Record,"."
                ELSE
                    WRITE(*,9721) "Processor ",MyRank," wrote the dots/lines for record ",Record, &
                                  ", layer ",IL1,", cell ",IL2,"/",IL3,"."
                ENDIF
            ENDIF
        ...
        IF(TRIM(ContourFileFormat).EQ."HWM-CSV")THEN
            ...
            DO
                READ(UNIT=37,FMT=*,END=999) JunkC
                I = I + 1
            ENDDO
999         CONTINUE
            REWIND(37)
            ...
            DO J = 1,I
                ...
                IF((KeepOpen(8).EQ.1).OR.(J.LT.I))THEN
                    Line = TRIM(Line)//" "//"-K"
                ENDIF
                Line = TRIM(Line)//" "//"-O >>"
                Line = TRIM(Line)//" "//TRIM(PlotName)//".ps"
 ...
 9724       CONTINUE
        ...
        IF((IfGoogle.EQ.1).OR.(IfGIS.GT.0))THEN
            ...
            OPEN(UNIT=33,FILE=TRIM(TempLabelsFile),ACTION="WRITE")
            WRITE(UNIT=33,FMT='(A,2X,A)') TRIM(XMin), TRIM(YMin)
            WRITE(UNIT=33,FMT='(A,2X,A)') TRIM(XMin), TRIM(YMax)
            WRITE(UNIT=33,FMT='(A,2X,A)') TRIM(XMax), TRIM(YMax)
            WRITE(UNIT=33,FMT='(A,2X,A)') TRIM(XMax), TRIM(YMin)
            CLOSE(UNIT=33,STATUS="KEEP")
        ...
        IF(IfPlotLabels.EQ.1)THEN
            ...
            DO I=1,NumLabels
                ...
                Line = TRIM(Line)//" "//"-G"//TRIM(LabelsColor)
                Line = TRIM(Line)//" "//"-S"//"Black"
                IF((KeepOpen(10).EQ.1).OR.(I.LT.NumLabels))THEN
                    Line = TRIM(Line)//" "//"-K"
                ENDIF
                Line = TRIM(Line)//" "//"-O >>"
                Line = TRIM(Line)//" "//TRIM(PlotName)//".ps"
            ...
            IF(Verbose.GE.3)THEN
                IF((IfGoogle.EQ.0).AND.(IfGIS.EQ.0))THEN
                    WRITE(*,9720) "Processor ",MyRank," wrote the labels for record ",Record,"."
                ELSE
                    WRITE(*,9721) "Processor ",MyRank," wrote the labels for record ",Record, &
                                  ", layer ",IL1,", cell ",IL2,"/",IL3,"."
                ENDIF
            ENDIF
...
!           IF(ABS(LongW-LatLonBuffer)<10.0)THEN
!               WRITE(UNIT=XMinBuf,FMT='(F5.2)') LongW-LatLonBuffer
!           ELSEIF(ABS(LongW-LatLonBuffer)<100.0)THEN
...
!           ENDIF
!           IF(ABS(LatS-LatLonBuffer)<10.0)THEN
!               WRITE(UNIT=YMinBuf,FMT='(F4.2)') LatS-LatLonBuffer
!           ELSEIF(ABS(LatS-LatLonBuffer)<100.0)THEN
!               WRITE(UNIT=YMinBuf,FMT='(F5.2)') LatS-LatLonBuffer
!           ELSE
!               WRITE(UNIT=YMinBuf,FMT='(F6.2)') LatS-LatLonBuffer
!           ENDIF
...
!                   " -R"//TRIM(XMinBuf)//"/"//TRIM(XMaxBuf)//"/"//TRIM(YMinBuf)//"/"//TRIM(YMaxBuf)// &
!                   " -N0.0")
...
!               CLOSE(UNIT=18,STATUS="DELETE")
            ...
            IF((IfGoogle.EQ.0).AND.(IfGIS.EQ.0))THEN
                ...
                IF(KeepOpen(11).EQ.1)THEN
                    Line = TRIM(Line)//" -K"
                ENDIF
                Line = TRIM(Line)//" -R0/1/0/1 -GBlack -N -O -Xa"//TRIM(SideBarXC)//"i"// &
            ...
            IF(Verbose.GE.3)THEN
                IF((IfGoogle.EQ.0).AND.(IfGIS.EQ.0))THEN
                    WRITE(*,9720) "Processor ",MyRank," wrote the vectors for record ",Record,"."
                ELSE
                    WRITE(*,9721) "Processor ",MyRank," wrote the vectors for record ",Record, &
                                  ", layer ",IL1,", cell ",IL2,"/",IL3,"."
                ENDIF
            ENDIF
       ...
       ! CREATE NEW FILE FORMAT for PARTICLE TRACKING
        IF(IfPlotPartTrack.EQ.1)THEN
              ...
              WRITE(UNIT=PartSizeSnp,FMT='(I1)')  PartSize
               ...
               IF(IfPlotInitialPositions.EQ.1)THEN
                Line = ""
                Line = TRIM(Line)//TRIM(Path)//"psxy"
            ...
            !    write(*,*) Line
             ...
             ! NOW add the current position
                Line = ""
                Line = TRIM(Line)//TRIM(Path)//"psxy"
                Line = TRIM(Line)//" "//TRIM(TempPath)//TRIM(PartTrackSnap)//".xyz"
                IF(IfGoogle.EQ.0)THEN
                    Line = TRIM(Line)//" "//"-JM"//TRIM(WidthC)//"i"
                ELSE
                    Line = TRIM(Line)//" "//"-JQ"//TRIM(CentralMeridianC)//"/"//TRIM(WidthC)//"i"
        ...
        IF(IfAddPlotLabel.EQ.1)THEN
            ...
            IF(Verbose.GE.3)THEN
                IF(IfGoogle.EQ.0)THEN
                    WRITE(*,9720) "Processor ",MyRank," wrote the plot label for record ",Record,"."
                ELSE
                    WRITE(*,9721) "Processor ",MyRank," wrote the plot label for record ",Record, &
                                  ", layer ",IL1,", cell ",IL2,"/",IL3,"."
                ENDIF
            ENDIF
        ELSEIF(IfAddPlotLabel.EQ.2)THEN
           OPEN(UNIT=27,FILE=PlotLabelInputFile,ACTION="READ")
           IF(Record.EQ.1)THEN
                 READ(27,'(A)') PlotLabel2
           ELSE
               DO i=1,Record-1
                 READ(27,'(A)') JunkC 
               ENDDO
          ...
          CLOSE(UNIT=27,STATUS="KEEP")
            WRITE(UNIT=PlotLabelFile,FMT='(A,I4.4,A)') "plotlabel.",Record,".txt"
            OPEN(UNIT=28,FILE=TRIM(TempPath)//TRIM(PlotLabelFile),ACTION="WRITE")
            WRITE(UNIT=28,FMT='(A,A)') "0.0 0.0 14 0 0 BC ",TRIM(PlotLabel2)
            CLOSE(UNIT=28,STATUS="KEEP")
        ...
        IF((IfAddTimeBar.EQ.1).AND.(IfGoogle.EQ.0).AND.(IfGIS.EQ.0))THEN
            ...
            IF(Verbose.GE.3)THEN
                IF(IfGoogle.EQ.0)THEN
                    WRITE(*,9720) "Processor ",MyRank," wrote the time bar for record ",Record,"."
                ELSE
                    WRITE(*,9721) "Processor ",MyRank," wrote the time bar for record ",Record, &
                                  ", layer ",IL1,", cell ",IL2,"/",IL3,"."
                ENDIF
            ENDIF
        ...
        IF((IfPlotLogo.EQ.1).AND.(IfGoogle.EQ.0).AND.(IfGIS.EQ.0))THEN
            ...
            IF(Verbose.GE.3)THEN
                IF(IfGoogle.EQ.0)THEN
                    WRITE(*,9720) "Processor ",MyRank," wrote the logo for record ",Record,"."
                ELSE
                    WRITE(*,9721) "Processor ",MyRank," wrote the logo for record ",Record, &
                                  ", layer ",IL1,", cell ",IL2,"/",IL3,"."
                ENDIF
            ENDIF
   ...
   !          WRITE(*,*) " have ps image now work weithit to make png  1"
    ...
    !      WRITE(*,*) " have ps image now work weithit to make png  2"
          ...
          !     Line = TRIM(Line)//" "//"-F"//TRIM(PlotName)//".png"
...
!               CALL SYSTEM("mv "//TRIM(TempPath)//TRIM(PlotName)//".eps .")
        ...
        IF(Verbose.GE.3)THEN
            IF((IfGoogle.EQ.0).AND.(IfGIS.EQ.0))THEN
                WRITE(*,9720) "Processor ",MyRank," created the raster images for record ",Record,"."
            ELSE
                WRITE(*,9721) "Processor ",MyRank," created the raster images for record ",Record, &
                              ", layer ",IL1,", cell ",IL2,"/",IL3,"."
            ENDIF
        ENDIF
...
SUBROUTINE WriteXYZFiles(Record)
        ...
        IF((IfPlotFilledContours.GE.1).OR.(IfPlotContourLines.GE.1))THEN
            ...
            IF((IfPlotFilledContours.EQ.1).OR.(IfPlotContourLines.EQ.1))THEN
                ...
                IF(TRIM(ContourFileFormat).EQ."ADCIRC-OUTPUT")THEN
                    ...
                    IF(Record.GT.1)THEN
                        ...
                        DO J=1,Record-1
                            ...
                            CALL ReadTimeStamp(19,TRIM(ContourFile1),CurrentTime,JunkR,NumNodes1,DefaultValue)
                ...
                ELSEIF((TRIM(ContourFileFormat).EQ."13-MANNING").OR.           &
                       ...
                       (TRIM(ContourFileFormat).EQ."13-EVIS").OR.              &
                       (TRIM(ContourFileFormat).EQ."13-STARTDRY"))THEN
     ...
     &                         .AND.(TRIM(ContourFileFormat).EQ."13-STARTDRY"))THEN
                ...
                IF(TRIM(ContourFileFormat).EQ."ADCIRC-OUTPUT")THEN
                    ...
                    IF(Record.GT.1)THEN
                        ...
                        DO J=1,Record-1
                            ...
                            CALL ReadTimeStamp(19,TRIM(ContourFile1),CurrentTime,JunkR,NumNodes1,DefaultValue)
                            CALL ReadTimeStamp(23,TRIM(ContourFile2),CurrentTime,JunkR,NumNodes2,DefaultValue)
     ...
     &                         .AND.(TRIM(ContourFileFormat).EQ."13-STARTDRY"))THEN
        ...
        IF(IfPlotVectors.EQ.1)THEN
            ...
            IF(Record.GT.1)THEN
                ...
                DO J=1,Record-1
                    ...
                    CALL ReadTimeStamp(20,TRIM(VectorFile),CurrentTime,JunkR,NumNodes1,DefaultValue)
            ...
            DO I=1,NumNodesLocal
                    IF(Vels1(XYZNodes(I)).GE.MinVectorToPlot) THEN
        ...
        IF(IfPlotPartTrack.EQ.1)THEN
         WRITE(*,*) "made it to make xyz part of program  1"
            WRITE(UNIT=PartTrackInitial,FMT='(A,A,I4.4,A)') TRIM(PartTrackFile), ".", Record, ".init"
            WRITE(UNIT=PartTrackSnap,FMT='(A,A,I4.4,A)') TRIM(PartTrackFile), ".", Record, ".snap"

            OPEN(UNIT=17,FILE=TRIM(TempPath)//TRIM(PartTrackInitial)//".xyz",ACTION="WRITE")
            OPEN(UNIT=18,FILE=TRIM(TempPath)//TRIM(PartTrackSnap)//".xyz",ACTION="WRITE")
            OPEN(UNIT=20,FILE=TRIM(PartTrackFile),ACTION="READ")
            READ(UNIT=20,FMT='(A)') JunkC
            READ(UNIT=20,FMT=*) NumRecs, NumParticles
            WRITE(*,*) NumRecs, NumParticles

            ALLOCATE(XpInit(1:NumParticles))
            ...
            IF(Record.GT.1)THEN
                DO J=1,Record-1
                   IF (J.EQ.1)THEN   ! get initial positions
                   ...
                   ENDIF
                 ENDDO
              ! now read in current record value
               READ(UNIT=20,FMT=*) CurrentTime, JunkR
               WRITE(*,*) Record, CurrentTime, JunkR
                ...
                ENDDO
            ELSE ! record not gt 1 record equals 1
                      READ(UNIT=20,FMT=*) CurrentTime, JunkR
                        DO I=1,NumParticles
                        CALL ReadNodeVals(20,3,JunkI,JunkR1,JunkR2,JunkR3)
                         XpInit(JunkI) = JunkR1
