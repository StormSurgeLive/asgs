...
cp $ASGSADVISORYDIR/$ENSTORM/$metalink $POSTADVISORYDIR/$ENSTORM
...
cp $ASGSADVISORYDIR/$ENSTORM/maxvwel.63 $POSTADVISORYDIR/$ENSTORM
...
END
#
cat $ASGSADVISORYDIR/post_notify.txt | mail -s "ASGS results available for storm $STORM advisory $ADVISORY on $HOSTNAME" $NOTIFY
