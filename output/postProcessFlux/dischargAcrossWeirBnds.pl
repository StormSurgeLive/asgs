...
while ($k<=$nbou) {
   ...
   while  ($j<=$nvell[$k]) {
      ...
      @data = split(/\s+/,$line); 
      $nbvv[$k][$j]=$data[0];
      if ($ibtype[$k]==3 or $ibtype[$k]==13 or $ibtype[$k]==23) {
         $barlanht[$k][$j]=$data[1];
	 $barlancfsp[$k][$j]=$data[2];
      }elsif ($ibtype[$k]==4 or $ibtype[$k]==24){
         $ibconn[$k][$j]=$data[1];
	 $barinht[$k][$j]=$data[2];
...
print "there are $nnstrs weir boundaries\n";
...
foreach my $nstr (@nstrList){
   ...
   foreach my $node (@nodestring){
      my $nx=$X[$node];
      my $ny=$Y[$node];
      ($nx,$ny)= &cppd($X[$node],$Y[$node],-91.,30.) if defined $useCPP;
       $NX[$i]=$nx;
       $NY[$i]=$ny;
      print "xy $nx $ny\n";
      $i++;
   ...
   # 2nd side of weir
   ######################################################A
   @nodestring= @{$ibconn[$nstr]};
   shift @nodestring;  # since the first value is undef
   print "$k 2nd boundary number $nstr\n";
   print "nodes: @nodestring\n";
   $i=1;
   @NX=();
   @NY=();
...
foreach my $nstr (@nstrList) { 
   print OUTFILE ",bnd_$nstr"."-s1 (m3s-1),bnd_$nstr"."-s2 (m3s-1)";
}
print OUTFILE "\n"; 
