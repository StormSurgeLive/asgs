#!/bin/bash
#------------------------------------------------------------------------
# prepare_dwlc_nowcast: assembles dwlc file from multiple nowcast dwlc files
#------------------------------------------------------------------------
...
if [ ! -e "$RUNPROPERTIES" ] ; then
    allMessage "prepare_dwlc_nowcast did NOT find the run.properties file."
    exit 1
fi
...
NNowcasts=2 # number of nowcasts to merge into one dwlc file
