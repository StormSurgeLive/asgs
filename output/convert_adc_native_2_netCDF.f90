...
IF (IARGC().EQ.0)THEN
   DO JJ=1,NUNITS
      ...
      IF (FILEEXIST) THEN
         ...
         CALL POSTNETCDF(CUNIT,FORTFILE,IgnoreDate,cmdLineDateProvided,SYYYY,SM,SD,SH,IERR)
         IF(IERR.EQ.1)THEN
            WRITE(*,*)'1'
            STOP
         END IF
...
ELSE
   ...
   DO WHILE(II.le.IARGC())
      ...
      CASE DEFAULT
         ...
         IF (FILEEXIST) THEN
!           WRITE(*,'(/A)') TRIM(FORTFILE)//' file found'
            CALL POSTNETCDF(FORTFILE(L-1:L),FORTFILE,IgnoreDate,cmdLineDateProvided,SYYYY,SM,SD,SH,IERR)
            IF(IERR.EQ.1)THEN
               WRITE(*,*)'1'
               STOP
            END IF
...
SUBROUTINE POSTNETCDF(CUNIT,FORTFILE,IgnoreDate,cmdLineDateProvided,SYYYY,SM,SD,SH,IERR)
!
!---------------------------------------------------------------------------C
!                                                                           C
...
CASE ('71','73')
   ...
   CALL STATUSCHECK(NF90_PUT_ATT(NCID=NCFILEID,VARID=ZETAID, NAME='units',        VALUES='inchesH2O'))
...
CASE ('72','74')
   ...
   CALL STATUSCHECK(NF90_PUT_ATT(NCID=NCFILEID,VARID=UBARID, NAME='long_name',    VALUES='E/W Wind Stress'))
   CALL STATUSCHECK(NF90_PUT_ATT(NCID=NCFILEID,VARID=UBARID, NAME='units',        VALUES='Pa/rho'))
   CALL STATUSCHECK(NF90_PUT_ATT(NCID=NCFILEID,VARID=UBARID, NAME='standard_name',VALUES='eastward_surface_wind_stress'))
   CALL STATUSCHECK(NF90_PUT_ATT(NCID=NCFILEID,VARID=UBARID, NAME='_FillValue',   VALUES=nf90_fill_float))
   CALL STATUSCHECK(NF90_PUT_ATT(NCID=NCFILEID,VARID=UBARID, NAME='missing_value',VALUES=-99999.))
   CALL STATUSCHECK(NF90_PUT_ATT(NCID=NCFILEID,VARID=UBARID, NAME='dry_value',    VALUES=-99999.))

   CALL STATUSCHECK(NF90_DEF_VAR(NCID=NCFILEID,NAME='TauY',  XTYPE=NF90_FLOAT,DIMIDS=(/MNPID, TIMID/),VARID=VBARID))
   CALL STATUSCHECK(NF90_PUT_ATT(NCID=NCFILEID,VARID=VBARID, NAME='long_name',    VALUES='N/S Wind Stress'))
   CALL STATUSCHECK(NF90_PUT_ATT(NCID=NCFILEID,VARID=VBARID, NAME='units',        VALUES='Pa/rho'))
   CALL STATUSCHECK(NF90_PUT_ATT(NCID=NCFILEID,VARID=VBARID, NAME='standard_name',VALUES='northward_surface_wind_stress'))
   CALL STATUSCHECK(NF90_PUT_ATT(NCID=NCFILEID,VARID=VBARID, NAME='_FillValue',   VALUES=nf90_fill_float))
   CALL STATUSCHECK(NF90_PUT_ATT(NCID=NCFILEID,VARID=VBARID, NAME='missing_value',VALUES=-99999.))
   CALL STATUSCHECK(NF90_PUT_ATT(NCID=NCFILEID,VARID=VBARID, NAME='dry_value',    VALUES=-99999.))
...
999 CONTINUE
WRITE(*,'(A)')'FORT FILE ENDED UNEXPECTEDLY.  THIS IS NOT TERMINAL, BUT THE NETCDF FILE IS SHORT.'
