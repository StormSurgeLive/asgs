...
if [[ $ENSTORM != namforecast ]]; then
   STORMNAME="namforecast${ADVISORY}"
else
...
for textfile in `ls *.txt`; do
   mv $textfile ${ADVISDIR}/$ENSTORM/plots 2>> ${SYSLOG}
done
for csvfile in `ls *.csv`; do
   mv $csvfile ${ADVISDIR}/$ENSTORM/plots 2>> ${SYSLOG}
done
...
for plotfile in `ls *.ps`; do
   ...
   ${IMAGEMAGICKPATH}/convert -rotate 90 $plotfile $pngname 2>> ${SYSLOG} 
done
numfiles=`ls *.png *.csv 2>> /dev/null | wc -l`
...
# RANGER SPECIFIC
# ----------------------------------
# jgf20100616: Commented out everything below as it now appears to be 
# redundant with our archiving script.
#
