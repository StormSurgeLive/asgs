...
if [ $MESHFILE = 0 ]; then
   MESHFILEOPT="-e spatial_5a_1r.ele"
else
   MESHFILEOPT="-i $MESHFILE"
fi
#
# write full circle Rmaxes data
$ADCIRCDIR/aswip -m 2 -a $ASWIPTIMESTEP $ASWIPFRAMEOPT
# write radial v and p
$ADCIRCDIR/aswip -m 2 -v $ASWIPTIMESTEP $ASWIPFRAMEOPT
...
echo "generating spatial wind field with $ADCIRCDIR/aswip -m 2 -s $ASWIPTIMESTEP $ASWIPFRAMEOPT $MESHFILEOPT"
$ADCIRCDIR/aswip -m 2 -s $ASWIPTIMESTEP $ASWIPFRAMEOPT $MESHFILEOPT # write spatial v and p
files=""
if [ $FRAME = "0" ]; then
