...
case(DOMAIN)
   ...
   case('swan_TM01.63')
      thisVarName = 'swan_TM01'
      call initFileMetaData(fn, thisVarName, 1, 1)
      call check(nf90_def_var(fn%nc_id,'swan_tmm10',nf90_double,nc_dimid,fn%ncds(1)%nc_varID))
      call check(nf90_put_att(fn%nc_id,fn%ncds(1)%nc_varID,'_fillValue',-99999.d0))
      call check(nf90_put_att(fn%nc_id,fn%ncds(1)%nc_varID,'long_name','mean absolute wave period'))
      call check(nf90_put_att(fn%nc_id,fn%ncds(1)%nc_varID,'standard_name','sea_surface_wave_mean_period_from_variance_spectral_density_first_frequency_moment'))
      call check(nf90_put_att(fn%nc_id,fn%ncds(1)%nc_varID,'coordinates','time y x'))
      call check(nf90_put_att(fn%nc_id,fn%ncds(1)%nc_varID,'location','node'))
      call check(nf90_put_att(fn%nc_id,fn%ncds(1)%nc_varID,'mesh','adcirc_mesh'))
      call check(nf90_put_att(fn%nc_id,fn%ncds(1)%nc_varID,'units','s'))
   case('swan_TM02.63')
      thisVarName = 'swan_TM02'
      call initFileMetaData(fn, thisVarName, 1, 1)
      call check(nf90_def_var(fn%nc_id,'swan_tmm10',nf90_double,nc_dimid,fn%ncds(1)%nc_varID))
      call check(nf90_put_att(fn%nc_id,fn%ncds(1)%nc_varID,'_fillValue',-99999.d0))
      call check(nf90_put_att(fn%nc_id,fn%ncds(1)%nc_varID,'long_name','mean absolute wave period'))
   ...
   case('swan_tps.63','swan_TPS.63') !tps
      ...
      call check(nf90_put_att(fn%nc_id,fn%ncds(1)%nc_varID,'units','s'))
   case('rads.64')
      thisVarName = 'radstress_x'
      call initFileMetaData(fn, thisVarName, 2, 1)
      fn%irtype = 2
      call check(nf90_def_var(fn%nc_id,'radstress_x',nf90_double,nc_dimid,fn%ncds(1)%nc_varID))
      call check(nf90_put_att(fn%nc_id,fn%ncds(1)%nc_varID,'_fillValue',-99999.d0))
      call check(nf90_put_att(fn%nc_id,fn%ncds(1)%nc_varID,'long_name','radiation stress gradient x component'))
      call check(nf90_put_att(fn%nc_id,fn%ncds(1)%nc_varID,'standard_name','radiation_stress_gradient_x'))
      call check(nf90_put_att(fn%nc_id,fn%ncds(1)%nc_varID,'coordinates','time y x'))
      call check(nf90_put_att(fn%nc_id,fn%ncds(1)%nc_varID,'location','node'))
      call check(nf90_put_att(fn%nc_id,fn%ncds(1)%nc_varID,'mesh','adcirc_mesh'))
      ...
      call check(nf90_put_att(fn%nc_id,fn%ncds(1)%nc_varID,'positive','east'))
      call check(nf90_put_att(fn%nc_id,fn%ncds(1)%nc_varID,'dry_value',-99999.0d0))
      fn%ncds(2)%varNameNetCDF = 'radstress_y'
      call check(nf90_def_var(fn%nc_id,'radstress_y',nf90_double,nc_dimid,fn%ncds(2)%nc_varID))
      call check(nf90_put_att(fn%nc_id,fn%ncds(2)%nc_varID,'_fillValue',-99999.d0))
      call check(nf90_put_att(fn%nc_id,fn%ncds(2)%nc_varID,'long_name','radiation stress gradient y component'))
      call check(nf90_put_att(fn%nc_id,fn%ncds(2)%nc_varID,'standard_name','radiation_stress_gradient_y'))
      call check(nf90_put_att(fn%nc_id,fn%ncds(2)%nc_varID,'coordinates','time y x'))
      call check(nf90_put_att(fn%nc_id,fn%ncds(2)%nc_varID,'location','node'))
      call check(nf90_put_att(fn%nc_id,fn%ncds(2)%nc_varID,'mesh','adcirc_mesh'))
      call check(nf90_put_att(fn%nc_id,fn%ncds(2)%nc_varID,'units','m-2 s-2'))
      call check(nf90_put_att(fn%nc_id,fn%ncds(2)%nc_varID,'positive','north'))
      call check(nf90_put_att(fn%nc_id,fn%ncds(2)%nc_varID,'dry_value',-99999.0d0))
