...
!    * boundaries that are only two nodes long
!    * levees that are below the height of the local topography 
!    * overlapping elements
!    * disjoint nodes
!    * nodes that have too many or too few connected elements for SWAN
!    * nodes repeated within a boundary (unless it is a closed boundary
!      and last node is the same as first)
!    * boundary nodes with no boundary condition
!    * boundary condition applied to nodes that are not on a boundary
! And provide the following helpful information
!    * various mesh properties, including extents and location of 
!      high resolution areas
!    * node IDs for visualization
!    * element IDs for visualization
!    * implicit diagonal coefficient (for zeroes or negatives)
!    * neighbor tables for visualization
!    * number of neighbors for each node
!    * max time step assuming 1m total water depth in normally
!      dry areas 
!
!--------------------------------------------------------------------------
...
!----------------------------------------------------------------------
!     B O U N D A R Y   A S S I G N M E N T   C H E C K 
!----------------------------------------------------------------------

integer, allocatable :: tbnode(m%np,2)
integer :: neinod1
integer :: neinum
integer :: neinump1
integer :: neinod2
...
do i=1,np
   do j=2,m%nneigh(i)
      ...
      do k=2,nneigh(neinod1)
         ! find yourself in that node's neighbor table
         if (neitab(neinod1,k).eq.i) then
            neinum=k          
         endif 
      end do                                       
      neinump1=neinum+1
      if (neinum.eq.nneigh(neinod1)) then
         neinump1=2
      endif
      ! find the next cw neighbor
      neinod2=neitab(neinod1,neinump1)               
      do k=2,nneigh(neinod2)
         if (neitab(neinod2,k).eq.neinod1) then
            neinum=k
         endif
      end do
      neinump1=neinum+1
      if (neinum.eq.nneigh(neinod2)) then
         neinump1=2
      endif
      neinod3=neitab(neinod2,neinump1)
      ! first leg was a boundary seg
...
!sort into connected boundary nodes
