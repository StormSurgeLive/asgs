...
#else
      subroutine netcdf2adcirc_main(InputArguments,InputLength) BIND(C)
...
#ifdef SINGLEEXE
      CALL ParseCPPCommandLineArgs(InputLength,InputArguments,argcount)
#else
...
#else
       write(6,'(A)') "ERROR: This code requires netCDF support."
#endif
...
!---------------------------------------------------------------------
#else
    subroutine netcdf2adcirc_main() BIND(C)
        write(6,*) "ERROR: This code must be compiled with netCDF support."
        stop
    end subroutine
#endif
