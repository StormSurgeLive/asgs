function [D,outtimefmt,turl]=GetNosWaterLevelViaSOSv4_8(varargin)
...
%     datumurlstr='epsg::5103';
    datumurlstr='%3D%3Durn:ogc:def:datum:epsg::5103';
else
%     datumurlstr=['noaa::',VertDatum];
    datumurlstr=['%3D%3Durn:ioos:def:datum:noaa::',VertDatum];
end
...
for cnt=1:nstatn
    ...
    if strcmpi(DataType,'VerifiedSixMinute')||strcmpi(DataType,'PreliminarySixMinute')||...
       ...
       strcmpi(DataType,'VerifiedDailyMean')
        if strcmpi(Units,'Feet')
            D(cnt).navail=numel(J.water_surface_height_above_reference_datum_feet_(curind));
            D(cnt).wl=J.water_surface_height_above_reference_datum_feet_(curind);
        elseif strcmpi(Units,'Meters')
    ...
    elseif strcmpi(DataType,'SixMinuteTidePredictions')||strcmpi(DataType,'HourlyTidePredictions')||...
        ...
        if strcmpi(Units,'Feet')
            D(cnt).navail=numel(J.sea_surface_height_amplitude_due_to_equilibrium_ocean_tide_feet_(curind));
            D(cnt).wl=J.sea_surface_height_amplitude_due_to_equilibrium_ocean_tide_feet_(curind);
        elseif strcmpi(Units,'Meters')
